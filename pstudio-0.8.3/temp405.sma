...
new g_Sounds_Girl[11][SOUND_SHORTPATH_MAXLEN] =
{
	"ms/girl_1", 
	"ms/girl_2", 
	"ms/girl_3", 
...
new g_C4Timer_Sounds[12][SOUND_SHORTPATH_MAXLEN] = 
{
	"ms/one",
	"ms/two",
	"ms/three",
	...
	if( fp )
	{
		...
		while( !feof(fp) )
		{
			...
			if( buffer[0] != ';' && parse(buffer, szSoundKey, charsmax(szSoundKey), szSoundFile, charsmax(szSoundFile)) == 2 )
			{
				...
				//Bomb Timer
				else if( equal(szSoundKey, "C4TimerSound") )
				{
					copy_sound(g_C4Timer_Sounds, charsmax(g_C4Timer_Sounds), szSoundFile)
