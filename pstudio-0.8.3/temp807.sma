...
public plugin_init()
{
	register_plugin("Парашют", "1.0", "KRoT@L/JTP10181 AND WAW555")
...
public HandleSay(id)
{
	...
	if (containi(args, "parachute") != -1) {
		if (gCStrike) client_print(id, print_chat, "комманды для парашута: buy_parachute, sell_parachute, give_parachute")
		client_print(id, print_chat, "Чтобы использовать парашут Вам нужно держать кнопку (ИСПОЛЬЗОВАТЬ) при падении")
	}
...
public buy_parachute(id)
{
	...
	if (!get_pcvar_num(pEnabled)) {
		client_print(id, print_chat, "Парашут плагин отключен")
		return PLUGIN_HANDLED
	}

	if (has_parachute[id]) {
		client_print(id, print_chat, "У вас уже есть парашут")
		return PLUGIN_HANDLED
	}
	...
	if (money < cost) {
		client_print(id, print_chat, "У вас не достаточно денег $%i", cost)
		return PLUGIN_HANDLED
	}

	cs_set_user_money(id, money - cost)
	client_print(id, print_chat, "Вы купили парашут для того, чтобы использовать - нажмите кнопку ИСПОЛЬЗОВАТЬ при падении.")
	has_parachute[id] = true
...
public sell_parachute(id)
{
	...
	if (!has_parachute[id]) {
		client_print(id, print_chat, "У вас не парашута, чтобы продавать!")
		return PLUGIN_HANDLED
	}

	if (access(id,PARACHUTE_LEVEL)) {
		client_print(id, print_chat, "Вы не можете продавать ваш парашут")
		return PLUGIN_HANDLED
	}
	...
	client_print(id, print_chat, "Вы продали ваш парашут за $%d", sellamt)
...
public give_parachute(id,args[])
{
	...
	if (!has_parachute[id]) {
		client_print(id, print_chat, "У вас нет парашута чтобы использовать")
		return PLUGIN_HANDLED
	}
	...
	if(has_parachute[player]) {
		client_print(id, print_chat, " %s Вы уже имеете парашут.", pl_name)
		return PLUGIN_HANDLED
	}
	...
	client_print(id, print_chat, "Вы отдали Ваш парашут за %s.", pl_name)
	client_print(player, print_chat, " %s Вам дали парашут.", id_name)
...
public admin_give_parachute(id, level, cid) {
	...
	if (!get_pcvar_num(pEnabled)) {
		client_print(id, print_chat, "Плагин парашут отключен")
		return PLUGIN_HANDLED
	}
	...
	if (arg[0]=='@'){
		...
		switch(get_cvar_num("amx_show_activity"))	{
			case 2:	client_print(0,print_chat,"Администратор %s: дал парашут игроку ^"%s^" players",name,arg[1])
			case 1:	client_print(0,print_chat,"Администратор: дал парашут игроку ^"%s^" players",arg[1])
		}

		console_print(id," Вы отдали парашут за ^"%s^" players",arg[1])
		...
		switch(get_cvar_num("amx_show_activity")) {
			case 2:	client_print(0,print_chat,"Администратор %s: дал парашут игроку ^"%s^"",name,name2)
			case 1:	client_print(0,print_chat,"Администратор: дал парашут игроку ^"%s^"",name2)
		}

		console_print(id,"Вы отдали парашут за ^"%s^"", name2)
		...
		if (velocity[2] < 0.0) {
			...
			if(para_ent[id] <= 0) {
				...
				if(para_ent[id] > 0) {
					...
					entity_set_model(para_ent[id], "models/ms/parachute.mdl")
