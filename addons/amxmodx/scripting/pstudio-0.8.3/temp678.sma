...
{
	precache_sound("waw555/knifeduelmenu.mp3");
	precache_sound("waw555/knifeduelstop1.mp3");
	precache_sound("waw555/knifeduelstart2.mp3");
...
public fnChallenge(id, opponent)
{
	...
	client_cmd(0,"mp3 play sound/waw555/knifeduelmenu");
	
	new iMenu = menu_create(szTitle, "menu_MainHandler", 0);
	menu_additem(iMenu, "Я мужик, я согласен!", "1", 0, -1);
	menu_additem(iMenu, "Я трус, чет я очкую!", "2", 0, -1);
...
public menu_MainHandler(id, menu, item)
{
	...
	switch(iKey)
	{
		case 1:
		{
			client_print(0, print_chat, " %s принял дуель на ножах! Он МУЖИК!", szName);
			fnStartDuel();
		...
		case 2:
		{
			client_print(0, print_chat, " %s отказался от дуели на ножах! Он ТРУС!", szName);
			client_cmd(0,"mp3 play sound/waw555/knifeduelstop1");
			g_bInChallenge = false;
...
public fnStartDuel()
{
	...
	client_cmd(0,"mp3 play sound/waw555/knifeduelstart2");
	server_cmd("amx_throwknives 0");
