...
new g_Girl_Sounds[7][SOUND_SHORTPATH_MAXLEN] =
{
	"ms/girl_1.wav", 
	"ms/girl_2.wav", 
	"ms/girl_3.wav", 
...
{
	set_dhudmessage(200, 0, 0, -1.0, 0.02, 0, 6.0, 6.0, 0.5, 0.15, -1, 0)
	show_dhudmessage(0, g_announce_sync, "%L", LANG_PLAYER, "PREPARE_FIGHT", g_roundCount)
}
...
public bombTimer()
{
	if (--g_C4Timer >= 0)
	{
		if( BombCountHUD )
		{
			...
			show_hudmessage(0, "ƒÓ ‚Á˚‚‡ ÓÒÚ‡ÎÓÒ¸: %d ÒÂÍ", g_C4Timer)
		}
		if (BombCountVoice)
		{
			if (g_C4Timer == 30)
